...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handle(HttpMessageNotReadableException ex, HttpServletRequest req) {
        var pd = problem(HttpStatus.BAD_REQUEST, "Malformed Request", ex.getMostSpecificCause().getMessage(),
                APP_ERRORS + "bad-request", traceId(req));
        return respond(pd);
    }
    ...
    public ResponseEntity<ProblemDetail> handle(MethodArgumentNotValidException ex, HttpServletRequest req) {
        var pd = problem(HttpStatus.BAD_REQUEST, "Validation Failed", "Request validation failed.",
                APP_ERRORS + "validation", traceId(req));
    ...
    public ResponseEntity<ProblemDetail> handle(ConstraintViolationException ex, HttpServletRequest req) {
        var errors = ex.getConstraintViolations().stream().map(cv -> Map.of(
    ...
    public ResponseEntity<ProblemDetail> handle(MethodArgumentTypeMismatchException ex, HttpServletRequest req) {
        var pd = problem(HttpStatus.BAD_REQUEST, "Invalid Parameter",
                "Parameter '%s' has invalid value '%s'".formatted(ex.getName(), ex.getValue()),
                APP_ERRORS + "invalid-parameter", traceId(req));
        return respond(pd);
    }
    ...
    public ResponseEntity<ProblemDetail> handleNotFound(RuntimeException ex, HttpServletRequest req) {
        var pd = problem(HttpStatus.NOT_FOUND, "Not Found", ex.getMessage() == null ? "Resource not found." : ex.getMessage(),
                APP_ERRORS + "not-found", traceId(req));
        return respond(pd);
    }
    ...
    public ResponseEntity<ProblemDetail> handleUnhandled(Exception ex, HttpServletRequest req) {
        var pd = problem(HttpStatus.INTERNAL_SERVER_ERROR, "Internal Server Error",
                "An unexpected error occurred.", APP_ERRORS + "internal", traceId(req));
