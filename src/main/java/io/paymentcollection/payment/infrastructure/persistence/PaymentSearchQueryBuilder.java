...
public class PaymentSearchQueryBuilder {
    ...
    public Query buildQuery(PaymentSearchRequest request) {
        ...
        if (request.getFromDate() != null && request.getToDate() != null) {
            ...
            mustQueries.add(Query.of(q -> q.range(r -> r.date(d -> {
                var b = new DateRangeQuery.Builder().field("date");
                if (request.getFromDate() != null) b.gte(request.getFromDate().toString());
                if (request.getToDate()   != null) b.lte(request.getToDate().toString());
        ...
        if (request.getStatus() != null && !request.getStatus().isEmpty()) {
            mustQueries.add(Query.of(q -> q.terms(t -> t
                    .field("status")
                    .terms(terms -> terms.value(request.getStatus().stream()
                            .map(FieldValue::of)
                            .toList())))));
        }
        ...
        if (request.getMinAmount() != null || request.getMaxAmount() != null) {
            mustQueries.add(new Query.Builder()
                    .range(r -> r.number(n -> {
                        NumberRangeQuery.Builder b = new NumberRangeQuery.Builder().field("amount");
        ...
        if (request.getCustomerId() != null) {
            mustQueries.add(Query.of(q -> q.term(t -> t.field("customerId").value(request.getCustomerId()))));
        }
